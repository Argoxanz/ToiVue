      ...
      <ion-list v-if="expense">
        <ion-item>
          <ion-label class="ion-text-wrap">
            <h2>{{ expense.name }}</h2>
            <p>Status: <ion-badge :color="statusColor(expense.status)">{{ expense.status }}</ion-badge></p>
            <p>Vendor: <a v-if="expense.vendor" :href="vendorLink(expense.vendor)" target="_blank" rel="noopener" aria-label="Vendor link">{{ expense.vendor }}</a><span v-else>-</span></p>
            <!-- Due date hidden for now -->
            <p>{{ currency }} {{ format(expense.amount_paid) }} / {{ currency }} {{ format(expense.amount_total) }}</p>
          </ion-label>
          <ion-button slot="end" size="small" @click="openAddPayment">Add payment</ion-button>
        </ion-item>
        ...
        <ion-item v-if="!payments.length">
          <ion-label>No payments yet</ion-label>
        </ion-item>
      </ion-list>
