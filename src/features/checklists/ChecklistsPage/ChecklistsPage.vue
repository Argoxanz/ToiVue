...
async function onDelete(id: number) {
  if (confirm(t('checklists.deleteThisChecklist'))) {
...
<template>
  <IonPage>
    ...
    <IonContent class="ion-padding">
      <IonCard>
        <IonCardHeader>
          <IonCardTitle>{{ $t('checklists.createChecklist') }}</IonCardTitle>
        </IonCardHeader>
        <IonCardContent>
          <form class="stack-md" @submit.prevent="onCreate">
            ...
            <IonItem>
              <IonLabel position="stacked">{{ $t("checklists.template") }} ({{ $t("checklists.optional") }})</IonLabel>
              <IonSelect v-model="form.template_id" interface="popover" placeholder="None">
                <IonSelectOption :value="null">{{ $t("checklists.none") }}</IonSelectOption>
                <IonSelectOption v-for="tpl in templates" :key="tpl.id" :value="tpl.id">{{ tpl.name }}</IonSelectOption>
              </IonSelect>
            </IonItem>
            <div>
              <IonButton type="submit" :disabled="loading || !form.name">{{ $t("common.create") }}</IonButton>
            </div>
          </form>
        </IonCardContent>
      </IonCard>
      ...
      <IonList v-if="list">
        <IonItem v-for="c in list.data" :key="c.id" button @click="router.push(`/checklists/${c.id}`)">
          ...
          <IonButton slot="end" color="danger" @click.stop="onDelete(c.id)">{{ $t("common.delete") }}</IonButton>
        </IonItem>
      </IonList>
